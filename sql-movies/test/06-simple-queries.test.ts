...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT (*) AS count, strftime('%Y', release_date) AS year FROM movies GROUP BY year ORDER BY year DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT (*) AS count, strftime('%m', time_created) AS month FROM movie_ratings GROUP BY month ORDER BY count DESC`;
      const result = await db.selectMultipleRows(query);
