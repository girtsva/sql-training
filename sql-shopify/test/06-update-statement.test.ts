...
describe("Update Statements", () => {
    ...
    it("should update one app title by app id", async done => {
        ...
        const query = `UPDATE ${APPS} SET title = 'QUICK VIEW' WHERE id = ${app.id}`;
        try {
    ...
    it("should update review developer reply and developer reply date by app id and author", async done => {
        ...
        const query = `UPDATE ${REVIEWS} SET developer_reply = 'test reply', developer_reply_date = '${timeStamp}' WHERE app_id = ${review.app_id} AND author = '${review.author}'`;
        try {
            await db.execute(query);
        } catch (e) { console.log(e); };
    ...
    it("should update all categories to uppercase", async done => {
        const query = `UPDATE ${CATEGORIES} SET title = UPPER(title) WHERE title != UPPER(title)`;
        try {
            await db.execute(query);
        } catch (e) { console.log(e); };
