...
describe("Queries Across Tables", () => {
    ...
    it("should select top 3 most common categories", async done => {
        const query = `
        SELECT COUNT (*) AS count, categories.title AS category FROM apps
    ...
    it("should select top 3 prices by appearance in apps and in price range from $5 to $10 inclusive (not matters monthly or one time payment)", async done => {
        const query = `
        SELECT
            COUNT (*) AS count, price, 
            CASE
                WHEN price LIKE '%/%' THEN CAST(substr(price, 2, instr(price,'/') -2) AS REAL)
                WHEN price LIKE '%one%' THEN CAST(substr(price, 2, instr(price,'o') -3) AS REAL)
                ELSE CAST('0' AS REAL)
            END casted_price
        FROM apps
        JOIN apps_pricing_plans ON apps.id = apps_pricing_plans.app_id
